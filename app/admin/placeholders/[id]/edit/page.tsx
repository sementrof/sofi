...
export default function EditPlaceholderPage() {
  ...
  const fetchPlaceholder = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/admin/placeholders/${params.id}`)
      if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`Ошибка при обновлении заглушки: ${error}`)
      }
    } catch (error) {
      console.error('Error updating placeholder:', error)
      alert('Ошибка при обновлении заглушки')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <h1 className="text-3xl font-serif font-bold text-foreground mb-8">Редактировать заглушку</h1>
